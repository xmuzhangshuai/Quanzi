...
public class BaseApplication extends Application {
	...
	public void onCreate() {
		...
		options.setNotificationEnable(PreferenceUtils.getInstance(myApplication).getSettingMsgNotification());
		// 设置收到消息是否有声音提示，默认为true
		options.setNoticeBySound(PreferenceUtils.getInstance(myApplication).getSettingMsgSound());
		// 设置收到消息是否震动 默认为true
		options.setNoticedByVibrate(PreferenceUtils.getInstance(myApplication).getSettingMsgVibrate());
		// 设置语音消息播放是否设置为扬声器播放 默认为true
		options.setUseSpeaker(PreferenceUtils.getInstance(myApplication).getSettingMsgSpeaker());
