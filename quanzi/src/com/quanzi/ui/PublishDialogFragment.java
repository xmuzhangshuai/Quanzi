 ...
 *
 * 项目名称：quanzi  
 * 类名称：PublishDialogFragment  
 * 类描述：点击发布按钮，跳出的帖子和活动的菜单。
 * @author zhangshuai
 * @date 创建时间：2015-4-27 上午11:12:42 
 *
 */
public class PublishDialogFragment extends DialogFragment implements OnItemClickListener {
	 ...
	 * @return
	 */
	static PublishDialogFragment newInstance() {
		PublishDialogFragment f = new PublishDialogFragment();
		return f;
	}
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		switch (position) {
		case 0:
			intent = new Intent(getActivity(), PublishPostActivity.class);
			getActivity().startActivity(intent);
			getActivity().overridePendingTransition(R.anim.zoomin, R.anim.zoomout);
			PublishDialogFragment.this.dismiss();
			break;
		case 1:
			intent = new Intent(getActivity(), PublishActActivity.class);
			getActivity().startActivity(intent);
			getActivity().overridePendingTransition(R.anim.zoomin, R.anim.zoomout);
			PublishDialogFragment.this.dismiss();
			break;
	...
	private class PublishDialogAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (menuitemList.get(position).equals("帖子")) {
				holder.imageView.setImageResource(R.drawable.takephoto);
			} else {
